...
export const LogisticsHero = ({
  ...
  <section className="relative isolate overflow-hidden bg-ink lg:flex lg:min-h-[calc(100svh-4rem)]">
    <div
      aria-hidden
    ...
    <div className="relative mx-auto grid w-full max-w-shell content-start grid-cols-1 gap-10 px-6 pt-16 pb-16 sm:pt-20 lg:pl-31.5 lg:pt-29 xl:grid-cols-[minmax(0,1fr)_21.25rem] xl:grid-rows-[1fr] xl:gap-16">
      <div>
        ...
        <HeroText
          ...
          headlineClassName="lg:text-display-sm 2xl:text-display-lg min-[1600px]:text-display-xl"
          subheadlineClassName="mt-7 max-w-170 text-lead leading-copy text-white/62 lg:text-lead"
        />
        ...
        >
          {facts.map((fact, i) => (
            ...
            >
              <span
                aria-hidden
                className="absolute top-1/2 left-0 hidden h-10 w-0.5 -translate-y-1/2 bg-accent-sky xl:block"
              />
              <MonoLabel aria-hidden className="text-accent-sky/60">
                {FACT_INDEX[i]}
              </MonoLabel>
              <dt className="text-caption leading-snug text-white/45">
                <Pending>{fact.label}</Pending>
              </dt>
              <dd className="m-0 text-title leading-none font-semibold tracking-snug text-white">
